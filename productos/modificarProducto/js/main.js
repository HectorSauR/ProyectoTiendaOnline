...
fetch('../../recursos/PHP/metodos/buscarProductoBd.php').then(res => res.json()).then(data => {
...
// btn.addEventListener("click", () =>{
  ...
  if(e.target.className == "editarS"){
   ...
   // alert(idprd)
     var categorias = contenedor.querySelector(".id-categoria > p").innerText;
  ...
  document.querySelector("#img_env").addEventListener("change", (e) => {
   // console.log(e.target.files)
    ...
    if (archivosSoportados.indexOf(imagen_env.type) != -1) {
      var path_img = "../../recursos/imagenes/productos/"+document.getElementById("nombre").value+"/"+imagen_env.name;
